		...
		<div class="row">
			<div class="col-lg-8 col-md-8">
				...
				<form action="/join_employee_info.do">
					...
					<hr>
					<h4>비밀번호</h4><span class="pwChk" style="color: green;"></span> 
					<div class="mt-10">
					...
					<span
						class="pwChkRe" style="color: orange;"></span>
					<hr>
					<h4>직원이름</h4>
					<div class="mt-10">
					...
					<span><input type="button"
						class="genric-btn info-border d-none d-lg-block"
					...
					<div class="input-group-icon mt-10">
						...
						<br> <input type="text" name="detailAddress"
							id="detail_address" placeholder="상세주소를 입력해주세요" required
							class="single-input">
					</div>
					<hr>
					<h4>생년월일</h4>
	...
	<script>
				...
				.on(
						...
						function(event) {
							if (/^[0-9a-z]+$/g.test($('[name=id]').val())) {
								...
								$('.idChk')
										.html(
												"<i class='bi bi-exclamation-circle'></i> 숫자, 영문만 입력 가능");
		...
		$('[name=pw]')
				.on(
						'keyup',
						function(event) {
							if (/(?=.*\d{1,50})(?=.*[~`!@#$%\^&*()-+=]{1,50})(?=.*[a-zA-Z]{2,50}).{8,50}$/g
									.test($('[name=pw]').val())) {
								$('.pwChk')
										.html(
												"<i class='bi bi-exclamation-circle'></i>");
								...
								$('.pwChk')
										.html(
												"<i class='bi bi-exclamation-circle'></i> 숫자, 특문 각 1회 이상, 영문은 2개 이상 사용하여 8자리 이상 입력");
